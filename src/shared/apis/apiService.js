...
export const fetchNearbyRestaurants = async (latitude, longitude, queryFor, cityName) => {
// try-catch block to handle any potential errors.
  try {
    ...
    const response = await axios.get(`textsearch/json?location=${latitude}%2C${longitude}&query=local%20${queryFor}%20in%20${cityName}&radius=5000&key=${API_KEY}`)
