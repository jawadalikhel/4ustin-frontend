...
export const fetchNearbyRestaurants = async (latitude, longitude, keyworkForPlace) => {
// try-catch block to handle any potential errors.
  try {
    ...
    // const response = await axios.get(`/nearbysearch/json?keyword=${keyworkForPlace}&location=${latitude},${longitude}&radius=2000&key=${API_KEY}`);
    const response = await axios.get(`textsearch/json?location=${latitude}%2C${longitude}&query=local%20${keyworkForPlace}%20in%20Austin&radius=5000&key=${API_KEY}`)
